...
PyObject* decode_usenet_chunks(PyObject* self, PyObject* args, PyObject* kwds) {
    ...
    if(num_bytes_reserved <= 0) {
        ...
        for(lp = 0; lp < lp_max; lp++) {
            num_bytes_reserved += (int)PyMapping_Size(PyList_GetItem(Py_input_list, lp));
        }
    }
...
static PyMethodDef SabyencMethods[] = {
    {
        "decode_usenet_chunks", decode_usenet_chunks, METH_VARARGS,
        "decode data"
    }, 
    {
        NULL, NULL, 0, NULL
...
static struct PyModuleDef sabyencmodule = {
...
PyInit_sabyenc(void)
{
    return PyModule_Create(&sabyencmodule);
}
